...
void CCoordinateMappingBasics::Update()
{
	...
	//get the Body Frame necessary to obtain Joint info
	if (SUCCEEDED(hr)) {
	...
	//All Frame info was successfully obtained, start processing 
    ...
    if (SUCCEEDED(hr))
    {
		...
		//get Body Frame data for joints
            ...
            for (int colorIndex = 0; colorIndex < (nColorWidth*nColorHeight); ++colorIndex)
            {
                ...
                if (p.X != -std::numeric_limits<float>::infinity() && p.Y != -std::numeric_limits<float>::infinity())
                {
                    ...
                    if ((depthX >= 0 && depthX < nDepthWidth) && (depthY >= 0 && depthY < nDepthHeight))
                    {
                        ...
                        // if we're tracking a player for the current pixel, draw this pixel green
                        if (player != 0xff)
                        {
...
/// </summary>
void CCoordinateMappingBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
	if (m_hWnd)
	{
